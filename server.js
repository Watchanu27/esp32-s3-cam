...
app.post('/upload', (req, res) => {
  ...
  // ESP32 สามารถส่ง HC-SR04 ผ่าน Header ได้
  const headerDistance =
    Number(req.get('X-Distance-CM'));

  if (!Number.isNaN(headerDistance)) {

    ultrasonicDistance =
      Number(headerDistance.toFixed(2));
  ...
  console.log(
    `📷 Camera frame received | Distance: ${
      Number.isNaN(headerDistance)
        ? 'N/A'
        : headerDistance.toFixed(2) + ' cm'
    }`
  );
...
function updateBoxState(distance) {
  ...
  if (lastBoxState === null) {
    ...
    // ไม่ส่ง LINE ตอนเปิด Server ครั้งแรก
...
app.post('/sensor', (req, res) => {
  ...
  // ===================================================
  // ตรวจสอบสถานะกล่อง
  // และส่ง LINE ถ้ามีของเข้ามาใหม่
  // ===================================================
